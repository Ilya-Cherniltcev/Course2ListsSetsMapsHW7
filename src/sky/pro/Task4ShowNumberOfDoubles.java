...
// ---------- преобразуем полученный список в map, где ключ - строка,
// ---------- а значение - число повторений данной строки -----
        Map<String, Integer> frazeMap = new HashMap<>();
        for (String s : fraze) {
// --------   первоначально число повторений каждой строки равно 0
            frazeMap.put(s, 0);
        }
        boolean isExist;
        int numberOfDoubles = 0;
        String key;
// ------------ подсчитываем число повторений каждой строки ------------
